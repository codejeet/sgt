...
- Added a realistic PMKB-style Ralph walkthrough to the README and wired `test_ralph_mode_realistic_rig_example.sh` into `test_ralph_mode_latest_main_proof.sh`, so latest-main proof now covers a documented operator-plausible rig example in addition to the lower-level Ralph regressions.
