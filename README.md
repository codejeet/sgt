...
- In that forced-pending state, `~/.sgt/plan-state/<rig>.json` also rewrites `completion.acceptance.status` to the effective pending status and preserves stale terminal plan metadata under `completion.acceptance.declared_*` for operator forensics.
