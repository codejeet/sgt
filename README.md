...
- Before creating a new issue, `sgt sling` checks recent open issues for the same symptom signature (normalized title) and required labels.
- If an open match is found within cooldown, dispatch is suppressed to avoid duplicate redispatch loops while a fix is still pending.
...
- Entering per-rig mode retires any leftover legacy shared `sgt-mayor` session and archives shared Mayor transient state under `~/.sgt/president/cutovers/<token>/`, so the cutover clears stale shared work state without touching durable board, blocker, plan-state, or polecat truth.
