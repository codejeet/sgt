...
grep -q 'retire any leftover legacy shared `sgt-mayor` session' "$DOC"
grep -q '^## Repo-Owned Proof Expectations$' "$DOC"
grep -q './test_president_cutover_migration_guard.sh' "$DOC"
...
grep -q 'retires any leftover legacy shared `sgt-mayor` session' "$README"
