...
if ! grep -q 'duplicate event ignored' "$OUT_FILE"; then
  echo "expected duplicate skip status line in refinery output" >&2
  exit 1
fi

LOG_FILE="$HOME_DIR/sgt/sgt.log"
if ! grep -q 'REFINERY_DUPLICATE_SKIP pr=#123 issue=#77' "$LOG_FILE"; then
